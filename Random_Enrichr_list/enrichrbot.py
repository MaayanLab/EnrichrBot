...
def reply_to_gwasbot(genes,tweet_id,text,screenshots):
...
for tweet in tweets:
  ...
  # set enrichr link
  enrichr_link = submit_to_enrichr(genes, 'GWASbot')
  # get enrichr screenshot
  browser = init_selenium(CHROMEDRIVER_PATH, windowSize='1200,1250')
  screenshots = [
    link_to_screenshot( link=enrichr_link, output=os.path.join(PTH, "screenshots", "gwas.png"), browser=browser, zoom='1')
    ]
  browser.quit()
  reply_to_gwasbot(genes,tweet_id,text,screenshots)
